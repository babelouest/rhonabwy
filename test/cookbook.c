...
static void jwe_test(const char * file_path) {
  ...
  if (json_object_get(j_content, "reproducible") == json_true()) {
    ...
    // This stunt is executed by a trained professional, do not try this at home!!!
    json_decref(jwe->j_header);
    jwe->j_header = NULL;
    jwe->j_header = json_copy(json_object_get(json_object_get(j_content, "encrypting_content"), "protected"));
    // /stunt
    
    ck_assert_ptr_ne(NULL, token = r_jwe_serialize(jwe, jwk_pubkey, 0));
    ck_assert_int_eq(5, split_string(token, ".", &token_split));
    ck_assert_str_eq(token_split[0], json_string_value(json_object_get(json_object_get(json_object_get(j_content, "output"), "json_flat"), "protected")));
    if (json_object_get(json_object_get(json_object_get(j_content, "output"), "json_flat"), "encrypted_key") != NULL) {
      ...
      ck_assert_int_eq(0, o_strlen(token_split[1]));
    }
