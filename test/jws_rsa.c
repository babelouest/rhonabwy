...
#define RS256_TOKEN_EMPTY_SIGNATURE "eyJhbGciOiJSUzI1NiIsImtpZCI6IjEifQ.VGhlIHRydWUgc2lnbiBvZiBpbnRlbGxpZ2VuY2UgaXMgbm90IGtub3dsZWRnZSBidXQgaW1hZ2luYXRpb24u."
...
START_TEST(test_rhonabwy_parse_token_invalid_content)
{
  ...
  ck_assert_int_eq(r_jws_parse(jws, RS256_TOKEN_INVALID_HEADER, 0), RHN_ERROR_PARAM);
  ck_assert_int_eq(r_jws_parse(jws, RS256_TOKEN_INVALID_HEADER_B64, 0), RHN_ERROR_PARAM);
  ck_assert_int_eq(r_jws_parse(jws, RS256_TOKEN_INVALID_PAYLOAD_B64, 0), RHN_ERROR_PARAM);
  ck_assert_int_eq(r_jws_parse(jws, RS256_TOKEN_INVALID_DOTS, 0), RHN_ERROR_PARAM);
...
START_TEST(test_rhonabwy_parse_token)
{
  ...
  ck_assert_int_eq(r_jws_parse(jws, RS256_TOKEN, 0), RHN_OK);
...
START_TEST(test_rhonabwy_verify_token_invalid)
{
  ...
  ck_assert_int_eq(r_jws_parse(jws, RS256_TOKEN_INVALID_SIGNATURE, 0), RHN_OK);
  ck_assert_int_eq(R_JWA_ALG_RS256, r_jws_get_alg(jws));
  ...
  ck_assert_int_eq(r_jws_init(&jws), RHN_OK);
  ck_assert_int_eq(r_jwk_init(&jwk_pubkey), RHN_OK);
  ck_assert_int_eq(r_jws_parse(jws, RS256_TOKEN_EMPTY_SIGNATURE, 0), RHN_ERROR_PARAM);
  ck_assert_int_eq(R_JWA_ALG_RS256, r_jws_get_alg(jws));
  ck_assert_int_eq(r_jwk_import_from_json_str(jwk_pubkey, jwk_pubkey_rsa_no_alg_str), RHN_OK);
  ck_assert_int_eq(r_jws_verify_signature(jws, jwk_pubkey, 0), RHN_ERROR_INVALID);
  r_jws_free(jws);
...
START_TEST(test_rhonabwy_set_alg_serialize_verify_ok)
{
  ...
  ck_assert_int_eq(r_jws_set_alg(jws_sign, R_JWA_ALG_RS256), RHN_OK);
  ...
  ck_assert_int_eq(r_jws_set_alg(jws_sign, R_JWA_ALG_RS384), RHN_OK);
  ck_assert_ptr_ne((token = r_jws_serialize(jws_sign, NULL, 0)), NULL);
  ...
  ck_assert_int_eq(r_jws_set_alg(jws_sign, R_JWA_ALG_RS512), RHN_OK);
  ck_assert_ptr_ne((token = r_jws_serialize(jws_sign, NULL, 0)), NULL);
...
static Suite *rhonabwy_suite(void)
{
  ...
  s = suite_create("Rhonabwy JWS RSA function tests");
...
int main(void)
{
  ...
  Suite *s;
  SRunner *sr;
  //y_init_logs("Rhonabwy", Y_LOG_MODE_CONSOLE, Y_LOG_LEVEL_DEBUG, NULL, "Starting Rhonabwy JWS RSA tests");
  r_global_init();
  s = rhonabwy_suite();
  sr = srunner_create(s);
