...
static gnutls_cipher_algorithm_t get_alg_from_enc(jwa_enc enc) {
...
static gnutls_mac_algorithm_t get_digest_from_enc(jwa_enc enc) {
...
static unsigned char * set_ptext_with_block(const unsigned char * data, size_t data_len, size_t * ptext_len, gnutls_cipher_algorithm_t alg) {
  ...
  if (*ptext_len) {
    if ((ptext = o_malloc(*ptext_len)) != NULL) {
      ...
      y_log_message(Y_LOG_LEVEL_ERROR, "set_ptext_with_block - Error allocating resources for ptext");
    }
  }
  return ptext;
}

static size_t get_key_size(jwa_enc enc) {
  size_t size = 0;
  switch (enc) {
    case R_JWA_ENC_A128CBC:
      size = 32;
    ...
    if (json_object_get(j_header, "jwk") != NULL) {
      ...
      if (r_jwk_import_from_json_t(jwk, json_object_get(j_header, "jwk")) != RHN_OK) {
        ...
        if (r_jwk_key_type(jwk, NULL, x5u_flags) & R_KEY_TYPE_HMAC) {
          if (r_jwks_append_jwk(jwe->jwks_privkey, jwk) != RHN_OK) {
            ret = RHN_ERROR;
          }
        }
      } else {
...
static void r_remove_padding(unsigned char * text, size_t * text_len, unsigned int block_size) {
...
static int r_compute_tag(jwe_t * jwe, unsigned char * cyphertext, size_t cyphertext_len, unsigned char * tag, size_t * tag_len) {
  int ret, res;
  unsigned char al[8], * compute_hmac = NULL;
  uint64_t aad_len;
  size_t hmac_size = 0, aad_size = o_strlen((const char *)jwe->header_b64url);
  gnutls_mac_algorithm_t mac = get_digest_from_enc(jwe->enc);
  ...
  if ((compute_hmac = o_malloc(aad_size+jwe->iv_len+cyphertext_len+8)) != NULL) {
    ...
    if (!(res = gnutls_hmac_fast(mac, jwe->key, 16, compute_hmac, hmac_size, tag))) {
      *tag_len = gnutls_hmac_get_len(mac)/2;
      ...
      y_log_message(Y_LOG_LEVEL_ERROR, "r_compute_tag - Error gnutls_hmac_fast: '%s'", gnutls_strerror(res));
      ret = RHN_ERROR;
    }
    o_free(compute_hmac);
  } else {
    y_log_message(Y_LOG_LEVEL_ERROR, "r_compute_tag - Error allocating resources for compute_hmac");
    ret = RHN_ERROR;
  }
  return ret;
}
...
int r_jwe_generate_cypher_key(jwe_t * jwe) {
  ...
  if (jwe != NULL && jwe->enc != R_JWA_ENC_UNKNOWN) {
    jwe->key_len = get_key_size(jwe->enc);
    o_free(jwe->key);
...
int r_jwe_generate_iv(jwe_t * jwe) {
  ...
  if (jwe != NULL && jwe->enc != R_JWA_ENC_UNKNOWN) {
    jwe->iv_len = gnutls_cipher_get_iv_size(get_alg_from_enc(jwe->enc));
    o_free(jwe->iv);
    jwe->iv = NULL;
    if (jwe->iv_len) {
...
int r_jwe_set_header_str_value(jwe_t * jwe, const char * key, const char * str_value) {
  ...
  if (jwe != NULL) {
    if ((ret = _r_header_set_str_value(jwe->j_header, key, str_value)) == RHN_OK) {
      o_free(jwe->header_b64url);
      jwe->header_b64url = NULL;
    }
    return ret;
...
int r_jwe_set_header_int_value(jwe_t * jwe, const char * key, int i_value) {
  ...
  if (jwe != NULL) {
    if ((ret = _r_header_set_int_value(jwe->j_header, key, i_value)) == RHN_OK) {
      o_free(jwe->header_b64url);
      jwe->header_b64url = NULL;
    }
    return ret;
...
int r_jwe_set_header_json_t_value(jwe_t * jwe, const char * key, json_t * j_value) {
  ...
  if (jwe != NULL) {
    if ((ret = _r_header_set_json_t_value(jwe->j_header, key, j_value)) == RHN_OK) {
      o_free(jwe->header_b64url);
      jwe->header_b64url = NULL;
    }
    return ret;
...
const char * r_jwe_get_header_str_value(jwe_t * jwe, const char * key) {
  if (jwe != NULL) {
    return _r_header_get_str_value(jwe->j_header, key);
  }
  return NULL;
}
...
int r_jwe_get_header_int_value(jwe_t * jwe, const char * key) {
  if (jwe != NULL) {
    return _r_header_get_int_value(jwe->j_header, key);
  }
  return 0;
}
...
json_t * r_jwe_get_header_json_t_value(jwe_t * jwe, const char * key) {
  if (jwe != NULL) {
    return _r_header_get_json_t_value(jwe->j_header, key);
  }
  return NULL;
}
...
json_t * r_jwe_get_full_header_json_t(jwe_t * jwe) {
  if (jwe != NULL) {
    return _r_header_get_full_json_t(jwe->j_header);
  }
  return NULL;
}
...
int r_jwe_add_keys(jwe_t * jwe, jwk_t * jwk_privkey, jwk_t * jwk_pubkey) {
  ...
  if (jwe != NULL && (jwk_privkey != NULL || jwk_pubkey != NULL)) {
    if (jwk_privkey != NULL) {
      if (r_jwks_empty(jwe->jwks_privkey) != RHN_OK || r_jwks_append_jwk(jwe->jwks_privkey, jwk_privkey) != RHN_OK) {
    ...
    if (jwk_pubkey != NULL) {
      if (r_jwks_empty(jwe->jwks_pubkey) != RHN_OK || r_jwks_append_jwk(jwe->jwks_pubkey, jwk_pubkey) != RHN_OK) {
...
int r_jwe_encrypt_payload(jwe_t * jwe) {
  ...
  if (jwe != NULL && jwe->payload != NULL && jwe->payload_len && jwe->enc != R_JWA_ENC_UNKNOWN && jwe->key != NULL && jwe->key_len && jwe->iv != NULL && jwe->iv_len) {
    ...
    ptext_len = gnutls_cipher_get_block_size(get_alg_from_enc(jwe->enc));
    if (0 == o_strcmp("DEF", json_string_value(json_object_get(jwe->j_header, "zip")))) {
      if ((text_zip = o_malloc(jwe->payload_len)) != NULL) {
        ...
        if (deflateInit(&defstream, Z_BEST_COMPRESSION) == Z_OK) {
          if (deflate(&defstream, Z_FINISH) == Z_STREAM_END) {
            if ((ptext = set_ptext_with_block(text_zip, defstream.total_out, &ptext_len, get_alg_from_enc(jwe->enc))) == NULL) {
              y_log_message(Y_LOG_LEVEL_ERROR, "r_jwe_encrypt_payload - Error set_ptext_with_block");
              ret = RHN_ERROR;
            }
          } else {
      ...
      if ((ptext = set_ptext_with_block(jwe->payload, jwe->payload_len, &ptext_len, get_alg_from_enc(jwe->enc))) == NULL) {
        y_log_message(Y_LOG_LEVEL_ERROR, "r_jwe_encrypt_payload - Error set_ptext_with_block");
        ret = RHN_ERROR;
      }
    }
    ...
    if (ret == RHN_OK) {
      ...
      key.size = gnutls_hmac_get_len(get_digest_from_enc(jwe->enc))/2;
      iv.data = jwe->iv;
      iv.size = jwe->iv_len;
      if (!(res = gnutls_cipher_init(&handle, get_alg_from_enc(jwe->enc), &key, &iv))) {
        if (!(res = gnutls_cipher_encrypt(handle, ptext, ptext_len))) {
          if ((ciphertext_b64url = o_malloc(2*ptext_len)) != NULL) {
            if (o_base64url_encode(ptext, ptext_len, ciphertext_b64url, &ciphertext_b64url_len)) {
              ...
              if (r_compute_tag(jwe, ptext, ptext_len, tag, &tag_len) == RHN_OK) {
                ...
                y_log_message(Y_LOG_LEVEL_ERROR, "r_jwe_encrypt_payload - Error r_compute_tag");
                ret = RHN_ERROR;
              }
            } else {
...
int r_jwe_decrypt_payload(jwe_t * jwe) {
  ...
  if (jwe != NULL && jwe->enc != R_JWA_ENC_UNKNOWN && (ciphertext_b64_len = o_strlen((const char *)jwe->ciphertext_b64url)) != 0 && o_strlen((const char *)jwe->iv_b64url) && jwe->key != NULL && jwe->key_len) {
    ...
    if (ret == RHN_OK) {
      ...
      iv.size = jwe->iv_len;
      if (!(res = gnutls_cipher_init(&handle, get_alg_from_enc(jwe->enc), &key, &iv))) {
        if (r_compute_tag(jwe, payload_enc, payload_enc_len, tag, &tag_len) == RHN_OK) {
          if ((tag_b64url = o_malloc(tag_len*2)) != NULL) {
            if (o_base64url_encode(tag, tag_len, tag_b64url, &tag_b64url_len)) {
          ...
          y_log_message(Y_LOG_LEVEL_ERROR, "r_jwe_decrypt_payload - Error r_compute_tag");
          ret = RHN_ERROR;
        }
        if (!(res = gnutls_cipher_decrypt(handle, payload_enc, payload_enc_len))) {
          r_remove_padding(payload_enc, &payload_enc_len, gnutls_cipher_get_block_size(get_alg_from_enc(jwe->enc)));
          if (0 == o_strcmp("DEF", json_string_value(json_object_get(jwe->j_header, "zip")))) {
...
int r_jwe_encrypt_key(jwe_t * jwe, jwk_t * jwk_pubkey, int x5u_flags) {
  ...
  unsigned int bits = 0;
  unsigned char * cypherkey_b64 = NULL;
  size_t cypherkey_b64_len = 0;
  ...
  if (jwk != NULL && jwe->key != NULL && jwe->key_len && jwe->alg != R_JWA_ALG_UNKNOWN && jwe->alg != R_JWA_ALG_NONE) {
    if ((g_pub = r_jwk_export_to_gnutls_pubkey(jwk, x5u_flags)) != NULL) {
      switch (jwe->alg) {
        case R_JWA_ALG_RSA1_5:
...
int r_jwe_decrypt_key(jwe_t * jwe, jwk_t * jwk_privkey, int x5u_flags) {
  ...
  unsigned int bits = 0;
  unsigned char * cypherkey_dec = NULL;
  size_t cypherkey_dec_len = 0;
  const char * k = NULL;
  ...
  if (jwk != NULL && o_strlen((const char *)jwe->encrypted_key_b64url) && jwe->alg != R_JWA_ALG_UNKNOWN && jwe->alg != R_JWA_ALG_NONE) {
    if ((g_priv = r_jwk_export_to_gnutls_privkey(jwk, x5u_flags)) != NULL) {
      switch (jwe->alg) {
        case R_JWA_ALG_RSA1_5:
          res = r_jwk_key_type(jwk, &bits, x5u_flags);
        ...
        case R_JWA_ALG_DIR:
          res = r_jwk_key_type(jwk, &bits, x5u_flags);
          if (res & R_KEY_TYPE_SYMMETRIC) {
            // Set jwe->key with base64url decoded "k" from jwk
            k = r_jwk_get_property_str(jwk, "k");
            if ((cypherkey_dec = o_malloc(o_strlen(k))) != NULL) {
              if (o_base64url_decode((const unsigned char *)k, o_strlen(k), cypherkey_dec, &cypherkey_dec_len)) {
                if ((res = r_jwe_set_cypher_key(jwe, cypherkey_dec, cypherkey_dec_len)) == RHN_OK) {
                  ret = RHN_OK;
                } else {
                  y_log_message(Y_LOG_LEVEL_ERROR, "r_jwe_decrypt_key - Error r_jwe_set_cypher_key (dir)");
                  ret = res;
                }
              } else {
                y_log_message(Y_LOG_LEVEL_ERROR, "r_jwe_decrypt_key - Error o_base64url_decode cypherkey_dec (dir)");
                ret = RHN_ERROR_PARAM;
              }
              o_free(cypherkey_dec);
            } else {
              y_log_message(Y_LOG_LEVEL_ERROR, "r_jwe_decrypt_key - Error allocating resources for cypherkey_dec (dir)");
              ret = RHN_ERROR_MEMORY;
            }
          } else {
            ret = RHN_ERROR_PARAM;
...
int r_jwe_parse(jwe_t * jwe, const char * jwe_str, int x5u_flags) {
  ...
  size_t header_len = 0, iv_len = 0, cypher_len = 0, split_size = 0;
  json_t * j_header = NULL;
  
  if (jwe != NULL && o_strlen(jwe_str)) {
    if ((split_size = split_string(jwe_str, ".", &str_array)) == 4 || split_size == 5) {
      ...
      if (o_base64url_decode((unsigned char *)str_array[0], o_strlen(str_array[0]), NULL, &header_len) && 
        ...
        do {
          ...
          if ((str_header = o_malloc(header_len+4)) == NULL) {
            y_log_message(Y_LOG_LEVEL_DEBUG, "r_jwe_parse - Error allocating resources for str_header");
            ret = RHN_ERROR_MEMORY;
            break;
          }
          
          if (!o_base64url_decode((unsigned char *)str_array[0], o_strlen(str_array[0]), (unsigned char *)str_header, &header_len)) {
            y_log_message(Y_LOG_LEVEL_DEBUG, "r_jwe_parse - Error o_base64url_decode str_header");
            ret = RHN_ERROR_PARAM;
            break;
          }
          ...
          if ((j_header = json_loads(str_header, JSON_DECODE_ANY, NULL)) == NULL) {
            y_log_message(Y_LOG_LEVEL_DEBUG, "r_jwe_parse - Error json_loads str_header");
            ret = RHN_ERROR_PARAM;
            break;
          }
          ...
          if ((iv = o_malloc(iv_len+4)) == NULL) {
            y_log_message(Y_LOG_LEVEL_DEBUG, "r_jwe_parse - Error allocating resources for iv");
            ret = RHN_ERROR_MEMORY;
            break;
          }
          
          if (!o_base64url_decode((unsigned char *)str_array[2], o_strlen(str_array[2]), iv, &iv_len)) {
            y_log_message(Y_LOG_LEVEL_DEBUG, "r_jwe_parse - Error o_base64url_decode iv");
            ret = RHN_ERROR_PARAM;
            break;
          }
          
          if (r_jwe_set_iv(jwe, iv, iv_len) != RHN_OK) {
            y_log_message(Y_LOG_LEVEL_DEBUG, "r_jwe_parse - Error r_jwe_set_iv");
            ret = RHN_ERROR;
            break;
          }
...
char * r_jwe_serialize(jwe_t * jwe, jwk_t * jwk_pubkey, int x5u_flags) {
  ...
  if (res == RHN_OK && r_jwe_encrypt_payload(jwe) == RHN_OK && r_jwe_encrypt_key(jwe, jwk_pubkey, x5u_flags) == RHN_OK) {
    jwe_str = msprintf("%s.%s.%s.%s.%s", 
                      jwe->header_b64url, 
                      jwe->encrypted_key_b64url,
                      o_strlen((const char *)jwe->iv_b64url)?(const char *)jwe->iv_b64url:"",
