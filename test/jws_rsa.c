...
START_TEST(test_rhonabwy_parse_token)
{
  ...
  ck_assert_int_eq(R_JWS_ALG_RS256, r_jws_get_alg(jws));
...
START_TEST(test_rhonabwy_set_alg_serialize_verify_ok)
{
  ...
  ck_assert_int_eq(r_jws_set_alg(jws_sign, R_JWS_ALG_RS256), RHN_OK);
  ...
  ck_assert_int_eq(r_jws_set_alg(jws_sign, R_JWS_ALG_RS384), RHN_OK);
  ck_assert_ptr_ne((token = r_jws_serialize(jws_sign, NULL, 0)), NULL);
  ...
  ck_assert_int_eq(r_jws_set_alg(jws_sign, R_JWS_ALG_RS512), RHN_OK);
  ck_assert_ptr_ne((token = r_jws_serialize(jws_sign, NULL, 0)), NULL);
